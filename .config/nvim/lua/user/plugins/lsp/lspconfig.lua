	...
	config = function()
		...
		vim.api.nvim_create_autocmd("LspAttach", {
			...
			callback = function(ev)
				...
				-- `grn` (rename), `gri` (implementations). Builtin `grr` uses the
				-- quickfix list; override it to use Telescope's popup picker instead.
		...
		vim.lsp.config("pyright", {
			...
			settings = {
				pyright = {
					-- Disable Pyright's organizing imports (let other tools handle it)
					disableOrganizeImports = true,
				},
				python = {
					analysis = {
						-- Disable type checking (let MyPy handle it)
						typeCheckingMode = "off",
						-- Disable diagnostics that overlap with Pylint/MyPy
						diagnosticMode = "openFilesOnly",
						-- Turn off specific checks that overlap
